...
/*   Updated: 2023/03/12 20:42:45 by yokitaga         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    multiple_pipes(t_info *info, t_environ *list)
{
    ...
    while (i < g_signal.pipe_count + 1)
    {
        ...
        else if (pid == 0)
        {
            ...
            {
                //=が含まれていない場合は抜ける
                //info->argv[j]に=が含まれている場合かつ環境変数が存在する場合は抜ける
                //info->argv[j]に=が含まれている場合かつ環境変数が存在しないはj++で次に進む
                //printf("%d\n", j);
                ...
                {
                    char *key;
                    key = ft_strndup(info->argv[j], ft_strchr_index(info->argv[j], '='));//=の前までをkeyに格納
                    if (ft_strncmp(search_env(key, info->list), NO_SUCH_ENV, ft_strlen(NO_SUCH_ENV)) != 0)//環境変数が存在する場合
                    {   
                        free(key);
                        break;
                    }
